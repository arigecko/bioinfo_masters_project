...
def map_introns_exons(isoforms_df, genome_f_path):
    ...
    for scaffold in scaffolds:
        ...
        for isoform in transcripts:
            ...
            gene_name = int(gene_name.replace("COWC_", ""))
...
def prepare_peaks_bed(peaks_data_path):
    # load the file with unassigned peaks. skip 30 rows s they just list the scaffold for wic we have data
    peaks_bed = pd.read_csv(peaks_data_path, sep="\t", names=['chrom', 'start', 'end'], skiprows=30)
...
def prepare_isoforms_df(isoforms_data_path):
    ...
    gene_ids = [x[1].split(' ') for x in attributes]
    gene_ids = [x[1][1:-2] for x in gene_ids]
    transcript_id = [x[0].split(' ') for x in attributes]
    transcript_id = [x[1][1:-1] for x in transcript_id]
...
def intron_exon_peaks(scaffold_df, scaffold, p_start, p_end, mapped_genome, mapped_genes):
    ...
    if category == 'intron' and transcript_intron:
        ...
        intron_genes = ["COWC_" + str(i) for i in intron_genes]
...
def assign_peaks(isoforms_data_path, peaks_data_path, genome_f_path, summary_tsv_path):
    ...
    for ind in peaks_bed.index:
        ...
        # check for peaks not overlapping with TSS
            ...
            category, genes, distance = intron_exon_peaks(scaffold_df, scaffold, p_start,
                                                          p_end, mapped_genome, mapped_genes)
            peaks_dict_summary[code] = [genes, distance, category]
...
plt.title('Fraction of peaks assigned to each category. Capsaspora', fontsize=15)
...
# Save the plot
fig.savefig('assigned_peaks_Capsaspora_new.png', dpi=fig.dpi)
