...
if len(sys.argv) != 5:
    print("Usage: python script.py <gtf_path> <atac_bed_file> <genome_fasta>")
    sys.exit(1)
...
def map_introns_exons(isoforms_df, genome_f_path):
    ...
    # keys are scaffolds values are lists of 0s of length equeal to the length of the corresponding sequence
    ...
    for scaffold in scaffolds:
        ...
        for isoform in transcripts:
            isoform = scaffold_df[(scaffold_df["transcript_id"] == isoform) & (scaffold_df["feature"] != 'CDS')]
            isoform_end = isoform.iat[0, 4]

            # store names of the genes
            gene_name = isoform.iat[0, 9]
            gene_name = int(gene_name.replace("AWHISG", ""))
...
def prepare_peaks_bed(peaks_data_path):
    # load the file with unassigned peaks. skip 636 rows as they just list the scaffold for wic we have data
    peaks_bed = pd.read_csv(peaks_data_path, sep="\t", names=['chrom', 'start', 'end'], skiprows=636)
...
def prepare_isoforms_df(isoforms_data_path):
    ...
    isof_col_names = ['seqname', 'source', 'feature', 'start', 'end', 'score', 'strand', 'frame', 'tr_id',
                      'transcript_id', 'g_id', 'gene_ids']
    isoforms_df = pd.read_csv(isoforms_data_path, sep=" ", header=None, names=isof_col_names)
    
    # prepare data for 2 new columns instead of attriutes one - gene ids and transcript ids
    gene_ids = isoforms_df['gene_ids'].to_list()
    for ii in range(len(gene_ids)):
        if gene_ids[ii][-1] == ';':
            gene_ids[ii] = gene_ids[ii][:-1]
    transcript_id = isoforms_df['transcript_id'].to_list()
    transcript_id = [x[:-1] for x in transcript_id]

    # remove not-needed columns and add new ones
    isoforms_df = isoforms_df.drop(['tr_id', 'g_id'], axis=1)
...
def get_closest_dist(scaffold_plus, scaffold_minus, p_start, p_end, midp=False):   # , intron = False):
    ...
    # Later the distances is adjusted and the half of the peak length is substracted form it
    if midp:
    ...
    if min(min_plus_val, min_minus_val) > 10000:
        ...
        if midp:
            ...
            if min_plus_val < min_minus_val:
                gene_ids, distance = scaffold_plus.loc[min_plus_ind, "gene_ids"], int(min_plus_val - peak_half)

            elif min_plus_val > min_minus_val:
                gene_ids, distance = scaffold_minus.loc[min_minus_ind, "gene_ids"], int(min_minus_val - peak_half)

            elif min_plus_val == min_minus_val:
                gene_ids = [scaffold_plus.loc[min_plus_ind, "gene_ids"], scaffold_minus.loc[min_minus_ind, "gene_ids"]]
                distance = int(min_plus_val - peak_half)
...
def intron_exon_peaks(scaffold_df, scaffold, p_start, p_end, mapped_genome, mapped_genes):
    ...
    if category == 'intron' and transcript_intron:
        ...
        intron_genes = ["AWHISG" + str(i) for i in intron_genes]
    ...
    # all other cases - peak ia overlapping with the end of the transcript; peak is not in a transcript;
    else:
...
def save_results(peaks_dict_summary, summary_tsv_path):
    ...
    peaks_assignment_dict = {'peak_code': peak_codes, 'gene_ids': genes_list_joined, 'dist': dist_l,
                             'category': category}
...
def assign_peaks(isoforms_data_path, peaks_data_path, genome_f_path, summary_tsv_path):
    # load and modify data
    isoforms_df = prepare_isoforms_df(isoforms_data_path)
    peaks_bed, peaks_dict_summary = prepare_peaks_bed(peaks_data_path)
    ...
    for ind in peaks_bed.index:
        ...
        # first check if the peak overlaps with any TSS of any genes(isoforms).
        # (here TSS is the first nucleotide at the  beginning of the gene)

        # check for both gene orientations
        dist_0_plus = scaffold_df[(scaffold_df['feature'] == 'transcript') & (scaffold_df['strand'] == "+") &
                                  (scaffold_df['start'] >= p_start) & (scaffold_df['start'] < p_end)]

        dist_0_minus = scaffold_df[(scaffold_df['feature'] == 'transcript') & (scaffold_df['strand'] == "-") &
                                   (scaffold_df['end'] > p_start) & (scaffold_df['end'] <= p_end)]
...
assign_peaks(isoforms_data_path=isoforms_data_path_sys, peaks_data_path=peaks_data_path_sys,
             genome_f_path=genome_f_path_sys, summary_tsv_path=summary_tsv_path_sys)
...
# categories = list(set(assigned_peaks_df['category'].to_list()))
tss_peaks = assigned_peaks_df[assigned_peaks_df['category'] == 'TSS']
tss_num = round((len(tss_peaks)/total_len * 100), 2)
intron_peaks = assigned_peaks_df[assigned_peaks_df['category'] == 'intron']
intron_num = round((len(intron_peaks)/total_len * 100), 2)
pure_exon_peaks = assigned_peaks_df[assigned_peaks_df['category'] == 'pure_exon']
pure_exon_num = round((len(pure_exon_peaks)/total_len * 100), 2)
pure_intergenic_peaks = assigned_peaks_df[assigned_peaks_df['category'] == 'pure_intergenic']
pure_intergenic_num = round((len(pure_intergenic_peaks)/total_len * 100), 2)
exon_intergenic_peaks = assigned_peaks_df[assigned_peaks_df['category'] == 'exon_intergenic']
exon_intergenic_num = round((len(exon_intergenic_peaks)/total_len * 100), 2)

# Creating dataset
categories = ['TSS', 'intron', 'pure exon', 'pure intergenic', 'exon/intergenic']

categ_val = [tss_num, intron_num, pure_exon_num, pure_intergenic_num, exon_intergenic_num]
...
category_counts = all_categories_df['Category'].value_counts().reset_index()
...
barplot = sns.barplot(x='Category', y='Percentage', data=category_counts, palette='viridis')

# Add title and labels
plt.title('Fraction of peaks assigned to each category. Abeoforma', fontsize=15)
...
# Save the plot
fig.savefig('assigned_peaks_Abeoforma.png', dpi=fig.dpi)
