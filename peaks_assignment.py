...
isoforms_data_path = sys.argv[1]
peaks_data_path = sys.argv[2]
genome_f_path = sys.argv[3]
summary_tsv_path = sys.argv[4]


def map_intorns_exons(isoforms_df,genome_f_path):
    ...
    for scaffold in scaffolds:
        ...
        for isoform in transcripts:
            ...
            isoform_start = isoform.iat[0, 3]
            isoform_end = isoform.iat[0, 4]

            # store names of the genes
            gene_name = isoform.iat[0, 8]
            gene_name = int(gene_name.replace("SUB2.g", ""))

            # store all coordinates for exonas and introns
            exons = []
            introns = []
            ...
            for intron in introns:
                for ii in range(intron[0], intron[1]+1, 1):
                    # to check if this position was previously assigned as exon and the gene names assigned to this position need to be reset
                    exon_check = False
                    if mapping[scaffold][ii] == 1:
                        exon_check = True
                    mapping[scaffold][ii] = 2
                    if (exon_check == True) or (len(mapping_genes[scaffold][ii]) == 1 and mapping_genes[scaffold][ii][0] == 0):
                        mapping_genes[scaffold][ii] = [gene_name]
                    elif gene_name in mapping_genes[scaffold][ii]:
                        continue
                    else:
...
def prepare_peaks_bed(peaks_data_path):
    ...
    peaks_bed = pd.read_csv(peaks_data_path, sep="\t", names=['chrom', 'start', 'end'], skiprows=114)
...
def prepare_isoforms_df(isoforms_data_path):
    ...
    attribues = isoforms_df['attributes'].to_list()
    # prepare data for 2 new columns instead of attriutes one - gene ids and transcript ids
    attribues = [x.split("; ") for x in attribues]
    gene_ids = [x[0].split(' ') for x in attribues]
    gene_ids = [x[1][1:-1] for x in gene_ids]
    transcript_id = [x[1].split(' ') for x in attribues]
...
def get_closest_dist(scaffold_plus, scaffold_minus, p_start, p_end, midp = False, intron = False):
    ...
    # the distance form the middle of the peak to all possile candidates.
...
def intron_exon_peaks(scaffold_df, peaks_bed, ind, scaffold, p_start, p_end, code, mapped_genome, mapped_genes):
    # return transcript_check, category, genes, distance,
    # mapping_genome = mapped_genome
    # mappin_genes = mapped_genes
    ...
    if category == 'intron' and transcript_intron:
        ...
        intron_genes = ["SUB2.g" + str(i) for i in intron_genes]
...
def save_results(peaks_dict_summary, summary_tsv_path):
    ...
    peaks_assinment_dict = {'peak_code': peak_codes, 'gene_ids': genes_list_joined, 'dist': dist_l,
                            'category': category}
    peaks_assinment_df = pd.DataFrame(peaks_assinment_dict)
    peaks_assinment_df
    # save te resulting df
    peaks_assinment_df.to_csv(summary_tsv_path, sep='\t', index=False)
...
def assign_peaks(isoforms_data_path, peaks_data_path, genome_f_path, summary_tsv_path):
    ...
    mapped_genome, mapped_genes = map_intorns_exons(isoforms_df,genome_f_path)
    ...
    for ind in peaks_bed.index:
        ...
        # conditions:
        transcript_c = scaffold_df['feature']=='transcript'
        scaffold_c = scaffold_df['seqname']==scaffold
        ...
        # check for peaks not overlapping with TSS
            ...
            category, genes, distance = intron_exon_peaks(scaffold_df, peaks_bed, ind,
                                                                              scaffold, p_start, p_end, code,
...
assign_peaks(isoforms_data_path, peaks_data_path, genome_f_path, summary_tsv_path)