...
# mapped_motifs_68 = ["MA0670.2", "MA0798.3", "MA0754.3", 'MA0147.4', 'MA1511.2']
...
mapped_motifs_68 = ["MA0670.2", "MA0798.3", "MA0754.3", "MA0807.1", "MA1991.2", "MA0664.2", "MA0151.1", "MA0752.2",
                    ...
                    "MA0105.4", "MA0593.2", "MA0708.3", "MA0875.2", "MA0147.4_comb", "MA0147.4_mut", "MA0147.4_mut2", "MA0147.4_ex", "MA000.2"] #, "MA000.2"]

# TESTING
# peaks_bed_path = 'D:\master_sponges\\peaks_TSS_info\\suberites\\atac_peaks.bed'
# motifs_translation_path = "D:\master_sponges\\atac_extract\\jaspar_meme_name_id.xlsx"
# fimo_output_path = "D:\master_sponges\\atac_extract\\fimo_sponge\\fimo_68_genome"
...
def fimo_tsv(fimo_path, motif_id):
    ...
    if file_size < 500:
        ...
        motif_tsv = pd.read_csv(fimo_path, sep='\t', usecols=range(5), skipfooter=3,
...
def motif_summary_update(peaks_bed, motif_tsv, motif_name):
    ...
    peaks_bed.iloc[:, -1] = peaks_bed.iloc[:, -1].fillna(0).astype('int')
...
for row in range(len(mapped_ids)):
    ...
    motif_1_summary = motif_summary_update(peaks_bed=peaks_bed, motif_tsv=motif_1_tsv, motif_name=motif_1)
    ...
    for col in range(len(mapped_ids)):
        ...
        if motif_1 == motif_2:
            motif_2 = motif2_n
            empty_col = [0] * len(motif_1_summary)
            motif_1_summary[motif_2] = empty_col

        else:
            # remove the overlap
            motif_2_tsv = remove_overlap(tsv_fixed=motif_1_tsv, tsv_check=motif_2_tsv)

            # trying to repeat the intial approach:
    ...
    for _ in range(iterations):
        ...
        for col in range(len(mapped_ids)):
            ...
            if motif_2 == motif_1: # for diagonal
                motif_2 = motif2_n
